  ...
  const renderInstructionalSection = () => (
    <div className="space-y-4">
      {instructions.map((item) => (
        <Card key={item.number} className="p-4 sm:p-6 border-2 border-teal-700 bg-blue-50 hover:shadow-lg transition">
          <div className="flex gap-3 sm:gap-4 items-start">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-teal-700 text-white flex items-center justify-center font-bold text-base sm:text-lg flex-shrink-0">
              {item.number}
            </div>
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-1 break-words">{item.title}</h3>
