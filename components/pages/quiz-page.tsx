...
/** One <pre> with inline line numbers for compact code blocks */
  ...
  const gradeAll = () => {
    ...
    for (const q of questions) {
      if (q.kind === "multiple-choice") {
        ...
        else if (picked === (q as any).correct)
          next[q.id] = { state: "correct" };
        else
          next[q.id] = {
            state: "incorrect",
            expected: (q as any).correct,
            got: picked,
          };
    ...
    <div className="min-h-screen bg-white">
                          ...
                          <div className="grid gap-3">
                            {(q as any).options?.map(
                              (
                                ...
                                const correctId = (q as any).correct;
      ...
      <div className="sticky bottom-0 left-0 right-0 border-t border-slate-200 bg-white backdrop-blur">
        <div className="mx-auto flex max-w-5xl justify-end gap-3 px-4 py-3 sm:px-6">
          <Button
            onClick={() => (locked ? onNext() : gradeAll())}
            className="rounded-lg bg-[#007B7C] px-6 py-2 font-bold text-white hover:brightness-110"
          >
            {locked ? "Next" : "Continue (Mark)"}
          </Button>
        </div>
      </div>
    </div>
